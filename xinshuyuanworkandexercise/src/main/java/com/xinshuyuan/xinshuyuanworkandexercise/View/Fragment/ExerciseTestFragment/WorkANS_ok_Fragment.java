...
public class WorkANS_ok_Fragment extends BaseFragment {
    public WorkANS_ok_Fragment(){}
  private String answerAnalysis;
    private View Mview;
    private Button return_button;
    private Button come_onbutton;
    ...
    public WorkANS_ok_Fragment(Activity layering_practice_activity, Handler layering_practice_handler,String flag) {
        super();
        this.exerciseHandler=layering_practice_handler;
...
//        Model=flag;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        Mview=inflater.inflate(R.layout.work_layout_ans_ok,container,false);
        inint();
        return Mview;
    }

    private void inint() {
        return_button= (Button) Mview.findViewById(R.id.return_button);
        come_onbutton= (Button) Mview.findViewById(R.id.come_onbutton);
        //返回列表
        ...
        //继续答题
        come_onbutton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //练习的继续答题
                if(Model.equals("exercise")){
                    ...
                    OkGo.post(XSYTools.getWorkUrl(HomeWorkConstantClass.GETPRACTICETEST_URL,layering_practice_activity)).params(map).execute(new StringCallback() {
                        @Override
                    ...
                    OkGo.post(XSYTools.getWorkUrl(HomeWorkConstantClass.GETTEST_URL,layering_practice_activity)).params(map).execute(new StringCallback() {
                        @Override
                        public void onSuccess(String s, Call call, Response response) {
                            XSYTools.i("得到下一题的json数据"+s);
                            //没题了走试题列表
                            if(s.equals("")){
                                ...
                                PerferenceService service=new PerferenceService(layering_practice_activity);
                                String ProjectId=service.getsharedPre().getString("ProjectId","");
                                long userId=service.getsharedPre().getLong("WORKuserId",0);
                                exerciseHandler.sendMessage(XSYTools.makeNewMessage(Common.SHOW_WORKLIST_INFO_FRAGMENT,ProjectId+","+userId));
                            }else{
                    ...
                    OkGo.post(XSYTools.getWorkUrl(HomeWorkConstantClass.GETPRACISEINFO_URL,layering_practice_activity)).params(getpraciseMap).execute(new StringCallback() {
                        @Override
                        public void onSuccess(String s, Call call, Response response) {
                            ...
                            try {
                                ...
                                OkGo.post(XSYTools.getWorkUrl(HomeWorkConstantClass.PRACTICES_URL,layering_practice_activity)).params(map).execute(new StringCallback() {
                                    @Override
                                    public void onSuccess(String s, Call call, Response response) {
